-- Migration: Add customer_id columns to tables if they don't exist

-- Add customer_id to cart_items if it doesn't exist
do $$
begin
  if not exists (
    ...
    -- Update check constraint to allow either session_id or customer_id
...
-- Add customer_id to wishlist_items if it doesn't exist
do $$
begin
  if not exists (
    select 1 from information_schema.columns 
...
-- Add customer_id to addresses if it doesn't exist
do $$
begin
  if not exists (
    select 1 from information_schema.columns 
...
-- Ensure customers table has auth_user_id if it doesn't exist
do $$
begin
  if not exists (
    select 1 from information_schema.columns 
