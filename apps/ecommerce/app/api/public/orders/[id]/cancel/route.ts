...
export async function POST(
  ...
  if (order.status === 'delivered') {
    return NextResponse.json({ error: 'Cannot cancel delivered orders' }, { status: 400 });
  }
  
  if (order.status === 'shipped') {
    return NextResponse.json({ error: 'Cannot cancel shipped orders. Please contact support for returns.' }, { status: 400 });
  }
